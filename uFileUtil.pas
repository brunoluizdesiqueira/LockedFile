...
function WhoIsLocking(pPath: String) : String;
...
begin
  ...
  begin
    RestartManagerLibrary := LoadLibrary(PChar(IncludeTrailingBackslash(GetSystemDir) + restartmanagerlib));
...
var
  ...
  key: PWideChar;
  res: integer;
  pnProcInfoNeeded: Integer;
  pnProcInfo: Integer;
  lpdwRebootReasons: Integer;
  resources: PArrayOfPWideChar;

begin
  res := RmStartSession(@handle, 0, key);
  if (res = ERROR_SUCCESS) then
  begin
    raise Exception.Create('Não foi possível iniciar a sessão de reinício. Não foi possível determinar arquivo bloqueador.');
  end;
  ...
  try
    ...
    resources[0] := PWideChar(pPath);

    res := RmRegisterResources(handle, 1, resources, 0, nil, 0, nil);

    if res <> ERROR_SUCCESS then
    begin
      raise Exception.Create('Não foi possível registrar o recurso.');
    end;
